      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="bg-emerald-50 p-4 border-b border-emerald-100 flex flex-wrap items-center justify-between gap-4 shrink-0">
          ...
          <div className="text-xs text-emerald-600 italic">
            * Showing price per pax for {nights} night{nights > 1 ? 's' : ''} (incl. 15% markup)
          </div>
        </div>
                ...
                <div className="p-4">
                  <div className="grid grid-cols-1 gap-3">
                    {season.prices.map((price, pIdx) => {
                      ...
                      const priceWithMarkup = basePrice * (1 + MARKUP_PERCENTAGE);
                      const pricePerPaxPerNight = priceWithMarkup / pax;
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-gray-500">
                                Room Price: {formatCurrency(Math.round(priceWithMarkup))} SAR / night
                                <span className="text-gray-400 ml-1">(Base: {basePrice})</span>
                              </div>
                            </div>
                          </div>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 text-center text-xs text-gray-500 shrink-0">
          * Calculated prices include 15% markup and are estimates in Indonesian Rupiah (IDR).
        </div>
      </div>
    </div>
  );
