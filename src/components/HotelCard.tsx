      ...
      <div className="bg-emerald-600 p-4 text-white">
        <div className="flex justify-between items-start gap-2">
          <button 
            ...
            className="text-xl font-bold truncate hover:underline text-left w-full focus:outline-none"
          >
            {hotel.name}
          </button>
          {hotel.stars && (
            <div className="flex items-center bg-emerald-700/50 px-2 py-1 rounded-lg shrink-0">
          ...
          {hotel.seasons.map((season, idx) => (
            <div key={idx} className="border-b border-gray-100 last:border-0 pb-4 last:pb-0">
              <div className="flex items-start gap-2 mb-3 text-gray-700">
                <Calendar className="w-4 h-4 mt-1 text-emerald-600 flex-shrink-0" />
                <span className="text-sm font-semibold uppercase tracking-wide">{season.range}</span>
              </div>
              
              <div className="grid grid-cols-3 gap-2">
                {season.prices.map((price, pIdx) => (
                  <div key={pIdx} className="bg-gray-50 rounded-lg p-2 text-center border border-gray-100">
